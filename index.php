...
<?php if (count($posts)) : ?>
  ...
  <ul>
    <li class="link"><a href="detail.php?id=<?php echo h($post['id'])?>"><?php echo h($post['title'])?>（作成日：<?php echo h($post['updated_at'])?>）</a>｜[<a href="edit.php?id=<?php echo h($post['id'])?>">編集</a>]｜[<a href="delete.php?id=<?php echo h($post['id'])?>">削除</a>]</li>
  </ul>
  <?php endforeach ?>
<?php else : ?>
  投稿された日記はありません。
